...
public abstract class SearchResults extends ActionBarActivity {
	...
	/**
	 * This populates the professorSet (returned) and hebrewTranslations HashMap
	 * (class variable).
	 * 
	 * @return
	 */
	public String[] parseHebrewProfessors() {
		...
		try {
			...
			while (infile.ready()) {
				...
				String hebName = StringEscapeUtils.unescapeJava(splitted[3])
						.trim();
		...
		String[] professorArrayStrings = Arrays.copyOf(professorArrayObjects,
				professorArrayObjects.length, String[].class);
	...
	// try {
	// String[] hebrewProfessorFiles =
	// getAssets().list("HebrewProfessorListingsEncoded");
	// for (int i = 0; i < hebrewProfessorFiles.length; i++) {
	// arrivedAtProfessors = false;
	// infile = new BufferedReader(new InputStreamReader(
	// getAssets().open("HebrewProfessorListingsEncoded/" +
	// hebrewProfessorFiles[i])));
	...
	// englishName = splittedOnSpace[2] + " " + splittedOnSpace[0] +
	// splittedOnSpace[1];
	// }
	// else {
	// englishName = splittedOnSpace[1] + " " + splittedOnSpace[2] + " " +
	// splittedOnSpace[0];
	// }
	// }
	// else {
	...
	// }
	// }
	// else if (inputLine.contains("GetEmployeeDetails") && arrivedAtProfessors
	// == true) {
	// start = inputLine.indexOf(">") + 1;
	...
	// else if (splittedOnSpace.length == 3) {
	// if (splittedOnSpace[0].indexOf("-") == splittedOnSpace[0].length() - 1) {
	// hebrewName = splittedOnSpace[2] + " " + splittedOnSpace[0] +
	// splittedOnSpace[1];
	// }
	// else {
	// hebrewName = splittedOnSpace[1] + " " + splittedOnSpace[2] + " " +
	// splittedOnSpace[0];
	// }
	// }
	// else {
	// throw new
	// IOException("Unfortunately, there is a quadruple hebrew name.");
	// }
	...
	// }
	// hebrewTranslations.put(StringEscapeUtils.unescapeHtml4(hebrewName),
	// englishName);
	// String faculty = hebrewProfessorFiles[i].substring(0,
	// hebrewProfessorFiles[i].indexOf(".html"));
	// String hebrewNameToUse = StringEscapeUtils.unescapeHtml4(hebrewName);
	// String hebrewNameEscapedJava =
	// StringEscapeUtils.escapeJava(hebrewNameToUse);
	// Professor p = new Professor(null, englishName, faculty,
	// hebrewNameEscapedJava, true);
	// professorsToInsert.add(p);
	// facultyMap.put(hebrewNameToUse, faculty);
	// //This will make the hebrew professor name in a new line after the
	// english name.
	...
	// name = capsFix2(name);
	// String faculty = courseFiles[i].substring(0,
	// courseFiles[i].indexOf(".html"));
